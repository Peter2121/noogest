...
#  theDbObj: DbObj = DbObj(len: 0, dbInfo: @[])  # internal db tracking collection
  theDbObj {.threadvar.} : DbObj  # internal db tracking collection

proc initDb(knd: DbKind): DbConnId =
  result = genOid()
  theDbObj = DbObj(len: 0, dbInfo: @[])
...
proc getDbId(o: DbConnId, dbo: DbObj): int {.noSideEffect.} =
  result = -1
  for i in 0..<dbo.len:
    if dbo.dbInfo[i].id == o: return i
...
template instantRowsIterate(conn, cmd, sqCmd, brktCmd: untyped; db: int,
  ...
  for r in `cmd`(dbConn, sqCmd(query), args):
    ...
    for i in 0..<r.len:
      instR.row[i] = `brktCmd`(r, i.int32)  # = r[i]
    instR.len = r.len
    yield instR
